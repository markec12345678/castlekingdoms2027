# Stronghold 2027 - Press Kit

> Informacije za medije, vsebinske ustvarjalce in skupnost za predstavitev projekta.

Zadnja posodobitev: 2026-08-01

---

## 📋 O projektu

**Stronghold 2027** je odprtokodna nadgradnja priljubljene igre Stronghold (2001, Firefly Studios), ki cilja na moderno izkušnjo z izboljšano grafiko, dodatnimi funkcionalnostmi in podporo za več jezikov. Projekt je fork odprtokodnega Stone Kingdoms projekta, ki je razvit z uradnim dovoljenjem Firefly Studios.

### Ključne informacije

| Atribut | Vrednost |
|---------|----------|
| **Ime projekta** | Stronghold 2027 |
| **Verzija** | 0.7.0-stronghold2027-alpha |
| **Ciljni datum izida** | 2027-09-01 |
| **Platforme** | Windows, macOS, Linux |
| **Licenca** | Apache 2.0 |
| **Tehnologija** | LÖVE 11.5 (Lua) |
| **Repozitorij** | [github.com/markec12345678/stronghold2027](https://github.com/markec12345678/stronghold2027) |
| **Ekipa** | 2 razvijalca |

---

## 🎮 Glavne lastnosti

### Gameplay
- **Klasična Stronghold izkušnja** - gradnja gradu, zbiranje virov, obramba
- **5 ekonomskih misij** s posebnimi izzivi
- **71 različnih zgradb** (stanovanja, kmetije, industrija, vojaške, cerkvene)
- **42 tipov enot** (delavci, vojaki, živali)
- **Free build mode** za kreativno gradnjo brez pritiska

### Tehnične izboljšave (2027)
- **Modding API** - ustvarjaj lastne mode z lifecycle hook-i
- **HD Shaderji** - bloom, color grading, vignette, dynamic lighting
- **Performance Profiler** - F3 za debug overlay
- **10 podprtih jezikov** - vključno s slovenščino
- **CI/CD pipeline** - avtomatsko testiranje vsakega commit-a

---

## 🌍 Podprti jeziki

| Jezik | Koda | Status |
|-------|------|--------|
| Angleščina | ENG | ✅ Popoln |
| Nemščina | DEU | ✅ Popoln |
| Francoščina | FRA | ✅ Popoln |
| Italijanščina | ITA | ✅ Popoln |
| Poljščina | POL | ✅ Popoln |
| Portugalščina | POR | ✅ Popoln |
| Ruščina | RUS | ✅ Popoln |
| Ukrajinščina | UKR | ✅ Popoln |
| Madžarščina | HUN | ✅ Popoln |
| **Slovenščina** | SLV | ✅ Popoln (NOVO) |
| Srbščina | SRP | ✅ Delno preveden (NOVO) |
| Grščina | ELL | ✅ Delno preveden (NOVO) |
| Bolgarščina | BUL | ✅ Delno preveden (NOVO) |
| Makedonščina | MKD | ✅ Delno preveden (NOVO) |
| Litovščina | LIT | ✅ Delno preveden (NOVO) |
| Latvijščina | LAV | ✅ Delno preveden (NOVO) |

---

## 📊 Statistika projekta

| Metric | Vrednost |
|--------|----------|
| Vrstic kode | 213,215 |
| Lua datoteke | 282 |
| Zgradbe | 71 |
| Enota | 42 |
| Krmilniki (Controllers) | 19+ |
| Test preverjanj | 343 (100% pass) |
| Commitov v upstream | 1,451 |
| LFS objektov | 1,151 (1.8 GB) |

---

## 🛣️ Roadmap

| Faza | Datum | Status |
|------|-------|--------|
| Faza 0: Priprava | 2026-08-01 | ✅ Končano |
| Faza 1: CI/CD + Slovenščina | 2026-08-15 | ✅ Končano |
| Faza 2: Bugfixi + Combat | 2026-09-15 | 🔄 V teku |
| Faza 3: HD grafika | 2027-03-15 | ⏳ Načrtovano |
| Faza 4: Nove vsebine | 2027-06-15 | ⏳ Načrtovano |
| Faza 5: Poliranje | 2027-08-15 | ⏳ Načrtovano |
| **Izid** | **2027-09-01** | 🎯 Cilj |

---

## 💬 Citat za medije

> "Stronghold 2027 je naša vizija moderne različice klasike iz leta 2001. Z odprtokodno skupnostjo in uradnim dovoljenjem Firefly Studios želimo ustvariti najboljšo različico Stronghold-a doslej - z izboljšano grafiko, podporo za več jezikov in zmogljivim modding sistemom."
>
> — Stronghold 2027 ekipa

---

## 📸 Screenshot guide

### Kako narediti kakovostne screenshote za medije

1. **Zaženi igro** v 1920×1080 ali višji resoluciji
2. **Vklopi HD shaderje** v nastavitvah (Effects > HD Shaders > Cinematic preset)
3. **Izklopi UI** za čiste krajše (F2 za toggle UI)
4. **Upostavi kamero** v izometričen pogled (default)
5. **Screenshot** s Ctrl+P (LÖVE default) ali sistemskim orodjem

### Priporočeni scenariji za screenshots

1. **Pregled gradu** - širok pogled na celoten grad z obzidjem
2. **Bitka** - vojaške enote v boju
3. **Ekonomija** - kmetije, kamnolomi, žage v delovanju
4. **Nočni pogled** - z dynamic lighting shaderjem
5. **Meni** - glavni meni z ozadjem gradu

---

## 🔗 Povezave

- **GitHub:** [github.com/markec12345678/stronghold2027](https://github.com/markec12345678/stronghold2027)
- **Upstream (Stone Kingdoms):** [gitlab.com/stone-kingdoms/stone-kingdoms](https://gitlab.com/stone-kingdoms/stone-kingdoms)
- **Originalna igra:** [strongholdcrusader.com](https://strongholdcrusader.com) (Firefly Studios)
- **LÖVE engine:** [love2d.org](https://love2d.org)

---

## 📞 Kontakt

- **GitHub Issues:** Za prijavo bugov in predloge
- **Pull Requests:** Dobrodošli za prispevke kode
- **Email:** (kontaktiraj preko GitHub profila)

---

## 📜 Pravna obvestila

- **Stronghold 2027** je neodvisen odprtokodni projekt
- **Licenca:** Apache 2.0 (dovoljuje komercialno uporabo z atribucijo)
- **Originalni asseti:** Last Firefly Studios, uporabljeni z dovoljenjem preko Stone Kingdoms projekta
- **Ime "Stronghold"** je zaščitena blagovna znamka Firefly Studios - projekt ni uradni Stronghold produkt

---

## 🙏 Zahvale

- **Firefly Studios** - za originalno igro in dovoljenje za uporabo assetov
- **Stone Kingdoms ekipa** - za odlično odprtokodno bazo
- **LÖVE community** - za odličen game engine
- **Crowdin prevajalci** - za obstoječe prevode
- **Stronghold 2027 ekipa** - slovenski prevod, CI/CD, modding API, HD shaderji

---

## 📈 Metrici za medije

### Razvojna aktivnost (avg. 2026)
- Commitov na teden: 5-10
- Aktivnih razvijalcev: 2
- Test pass rate: 100%
- Issue resolution time: <48h (kritični)

### Skupnost (predvideno po izidu)
- Cilj GitHub zvezdic: 500+ (v prvem letu)
- Cilj Discord članov: 200+ (v prvih 6 mesecih)
- Cilj prevajalcev: 15+ jezikov (v prvem letu)
