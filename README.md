...
[![Version](https://img.shields.io/badge/version-3.11.969-blue.svg)](https://github.com/markec12345678/castlekingdoms2027/releases)
...
- **Zadnja izdaja**: [v3.11.969](https://github.com/markec12345678/castlekingdoms2027/releases)
- **.love datoteka**: `castlekingdoms2027-v3.11.969.love` (34 MB brez LFS, 305 MB z LFS)
- **Changelog**: [CHANGELOG.md](CHANGELOG.md) — 987 Royal sistemov, 1645 Lua datotek, +AUTO-SAVE OVERLAY HOVER TOOLTIP (hover prikže status, timer, save count, Royal stats) + ROYAL SYSTEMS PANEL HOVER + MARKET DASHBOARD HOVER + CUSTOM PRESET DELETION + CUSTOM PRESETS + MULTI-SELECT PERSISTENCE + CONFIG PRESETS + EXPORT/IMPORT + MULTI-SELECT + MARKET DASHBOARD QUICK-JUMP + BOOKMARKS + KEYBOARD NAVIGATION + MINIMAP DRAG + PROGRESS BAR + STATS SUMMARY + STATE FILTER + DEPTH-BASED SORTING + ARROWS + DEPTH INDICATOR + MINIMAP + CLICK-TO-JUMP + PATH HIGHLIGHT + SEARCH/FILTER + CLICK-TO-FOCUS + GRAF z bezier krivuljami + hover tooltip + 65 deps v 25 verigah + 100% mouse/wheel + overlay + DynamicMarket + Save/Load + 7 grafov
...
| GLSL shaderji | 12 |
| Jezikov | 32 |
| Verzij | 140+ (v1.7.9 → v3.11.969) |
| Bug popravkov | 155 (90 krogov pregleda) |
| Syntax pass rate | 1645/1645 (100%) |
| PNG assetov | 1.206 |
| **Royal sistemov** | **987 (v3.11.969)** |
| **Royal Tech Tree Panel** | **Ctrl+Shift+G (GRAF + FILTER L + SORT S + ARROWS A + DEPTH D + MINIMAP M + CLICK-TO-JUMP + PATH HIGHLIGHT T + SEARCH/FILTER + CLICK-TO-FOCUS + hover tooltip + G toggle za tekst + 65 deps v 25 verigah, 8 multi-prereq, scrollable)** |
...
| **Royal Auto-Save Panel** | **Ctrl+U (status, interval, force save, reset pozicije, OPACITY SLIDER, WHEEL ZA INTERVAL)** |
| **Royal Auto-Save Overlay** | **always-on HUD (status, timer, progress, drag-to-move, hide/show persisted, opacity wheel persisted)** |
