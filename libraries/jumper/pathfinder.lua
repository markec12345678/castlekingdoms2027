...
if (...) then
        ...
        for x = self._grid._max_x, self._grid._min_x, -1 do
            for y = self._grid._max_y, self._grid._min_y, -1 do
                ...
                if self._grid:isWalkableAt(x, y, self._walkable) then
                    ...
                    node._clearance[self._walkable] = 0
                end
            end
        end
    ...
    function Pathfinder:getPath(startX, startY, endXorEndNodes, endY, clearance, multipleEndNodes)
        self:reset()
        local startNode = self._grid:getNodeAt(startX, startY)
        local endNode = endXorEndNodes
        if not multipleEndNodes then
            endNode = self._grid:getNodeAt(endXorEndNodes, endY)
        else
            local newEndNodes = {}
            for _, v in ipairs(endXorEndNodes) do
                newEndNodes[#newEndNodes + 1] = self._grid:getNodeAt(v.x, v.y)
            end
            endNode = newEndNodes
        end
        ...
        local _endNode = Finders[self._finder](self, startNode, endNode, clearance, toClear, multipleEndNodes)
        if _endNode then
