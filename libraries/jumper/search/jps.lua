...
if (...) then
  ...
  --]]
    local function jump(finder, node, parent, endNodes, clearance, multipleEndNodes)
        ...
        if multipleEndNodes then
            for _, nd in ipairs(endNodes) do
                if node._x == nd._x and node._y == nd._y then return node end
            end
        else
            if node._x == endNodes._x and node._y == endNodes._y then return node end
        end
        ...
        if dx ~= 0 and dy ~= 0 then
            if jump(finder, finder._grid:getNodeAt(x + dx, y), node, endNodes, clearance, multipleEndNodes) then return node end
            if jump(finder, finder._grid:getNodeAt(x, y + dy), node, endNodes, clearance, multipleEndNodes) then return node end
        end
        ...
        if finder._allowDiagonal then
            if isWalkableAt(x + dx, y, finder._walkable, clearance) or isWalkableAt(x, y + dy, finder._walkable, clearance) then
                return jump(finder, finder._grid:getNodeAt(x + dx, y + dy), node, endNodes, clearance, multipleEndNodes)
            end
        end
    end
  ...
  --]]
    local function identifySuccessors(finder, openList, node, endNodes, clearance, toClear, multipleEndNodes)
        ...
        for i = #neighbours, 1, -1 do
            ...
            local jumpNode = jump(finder, neighbour, node, endNodes, clearance, multipleEndNodes)
                    ...
                    if not jumpNode._opened or newG < jumpNode._g then
                        ...
                        if multipleEndNodes then
                            jumpNode._h = jumpNode._h or
                                (finder._heuristic(jumpNode, endNodes[1]))
                        else
                            jumpNode._h = jumpNode._h or
                                (finder._heuristic(jumpNode, endNodes))
                        end
    ...
    return function(finder, startNode, endNode, clearance, toClear, multipleEndNodes)
        ...
        while not openList:empty() do
            ...
            if multipleEndNodes then
                for _, v in ipairs(endNode) do
                    if node._x == v._x and node._y == v._y then
                        return node
                    end
                end
            else
            ...
            identifySuccessors(finder, openList, node, endNode, clearance, toClear, multipleEndNodes)
        end
