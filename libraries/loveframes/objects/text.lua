...
--]] ---------------------------------------------------------
    ...
    function newobject:setAsTitle()
        ...
        local directives = skin.directives
        if directives then
            local text_default_color
            if self.title then
                text_default_color = directives.text_title_color
            else
                text_default_color = directives.text_default_color
            end
            local text_default_shadowcolor = directives.text_default_shadowcolor
            local text_default_font = directives.text_default_font
            if text_default_color then
...
--]] ---------------------------------------------------------
    function newobject:SetText(t)
        ...
        for k, v in ipairs(textdata) do
            ...
            local color = v.color
            if detectlinks then
                if loveframes.utf8.len(text) > 7 and
                    (loveframes.utf8.sub(text, 1, 7) == "http://" or loveframes.utf8.sub(text, 1, 8) == "https://") then
                    v.link = true
                end
            end
