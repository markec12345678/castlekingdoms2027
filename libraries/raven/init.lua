...
local remove_all_metatables = function(item, path)
    if path[#path] ~= inspect.METATABLE and type(path) ~= "number" then
        return item
    end
end
...
local function backtrace(level)
    ...
    while true do
        ...
        if info.currentline ~= -1 and not string.find(info.short_src, "ErrorHandler.lua") and not string.find(info.short_src, "love \"boot.lua\"") then
            if first then
                local i = 1
                while true do
                    ...
                    if type(v) == "table" then
                        if n == "self" then
                            for k, sv in pairs(v) do
                                valContext["Self"][k] = inspect(sv, { depth = 0, process = remove_all_metatables })
                            end
                        else
                            valContext["Values"][n] = inspect(v, { depth = 1, process = remove_all_metatables })
                        end
                    else
                        valContext["Values"][n] = tostring(v)
                    end
                    i = i + 1
                end
                if not next(valContext["Values"]) then
                    valContext["Values"] = nil
                end
                if not next(valContext["Self"]) then
                    valContext["Self"] = nil
                end
                first = false
            end
            local lines = love.filesystem.lines(info.short_src)
            local contextLine
            local preContextLines = {}
            local afterContextLines = {}
            local thisLine = 0

            for line in lines do
                thisLine = thisLine + 1
                if thisLine == info.currentline then
                    contextLine = line
                elseif thisLine < info.currentline and info.currentline - thisLine < 5 then
                    preContextLines[#preContextLines + 1] = line
                elseif thisLine > info.currentline and thisLine - info.currentline < 5 then
                    afterContextLines[#afterContextLines + 1] = line
                end
                if #afterContextLines >= 4 then
                    break
                end
            end
            table_insert(frames, 1, {
                ...
                post_context = afterContextLines,
                platform = "lua"
            })
        end
        level = level + 1
    end
    return valContext, { frames = frames }
end

-- error_catcher: used to catch an error from xpcall and return a correct
-- error message
local function error_catcher(err)
    local valContext, backtrace = backtrace(catcher_trace_level)
    return {
        message = err,
        contexts = valContext,
...
function _M.new(conf)
    ...
    if not conf.sender then return nil end
...
function raven_mt:captureException(exception, conf)
    ...
    local context, stacktrace = backtrace(trace_level)
    exception[1].stacktrace = stacktrace
