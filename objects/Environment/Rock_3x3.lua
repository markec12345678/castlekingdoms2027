...
function Rock:initialize(gx, gy, type)
    ...
    local tiles, quad_array = _G.indexBuildingQuads("rocks_3x3tile (" .. love.math.random(17, 32) .. ")", false, 3)
    ...
    for tile = 1, tiles do
        Rock_alias:new(quad_array[tile], self.gx + tile - 1, self.gy + tiles, self,
            center_tile_offset_y - 14 + 8 - 32 + 8 * tile)
    end
    ...
    for tile = 1, tiles do
        Rock_alias:new(quad_array[tiles + 1 + tile], self.gx + tiles, self.gy + (tiles - tile), self,
            center_tile_offset_y - 14 - 32 + 8 * (tiles - tile) + 8 + 8, 16)
    end
    Structure.render(self)
end
