...
function ChurchAlias:initialize(tile, gx, gy, parent, offsetY, offsetX)
...
function ChurchAlias:serialize()
    local data = {}
...
function ChurchAlias.static:deserialize(data)
...
Church.static.LENGTH = 9
Church.static.HEIGHT = 17
...
function Church:initialize(gx, gy)
    ...
    for tile = 1, tiles do
        local hsl = ChurchAlias:new(quadArray[tile], self.gx, self.gy + (tiles - tile + 1), self,
    ...
    for xx = 0, Church.static.WIDTH - 1 do
        for yy = 0, Church.static.LENGTH - 1 do
            if not _G.objectFromSubclassAtGlobal(self.gx + xx, self.gy + yy, Structure) then
                ChurchAlias:new(tileQuads["empty"], self.gx + xx, self.gy + yy, self, 0, 0)
            end
        end
    end
...
function Church:destroy()
    for xx = 0, Church.static.WIDTH - 1 do
        for yy = 0, Church.static.LENGTH - 1 do
            if not _G.objectFromSubclassAtGlobal(self.gx + xx, self.gy + yy, Structure) then
                _G.DestructionController:destroyAtLocation(self.gx + xx, self.gy + yy, false, true)
            end
        end
    end
    self.float:destroy()
...
function Church.static:deserialize(data)
    local obj = self:allocate()
    obj:load(data)
    return obj
end
